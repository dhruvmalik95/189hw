...
class Project2D(): 
	...
	def project_data(self,X,Y,white=None): 
		...
		plt.scatter(p_a[:,0],p_a[:,1],label = 'apple', color="r")
		plt.scatter(p_b[:,0],p_b[:,1],label = 'banana', color="g")
		plt.scatter(p_c[:,0],p_c[:,1],label = 'eggplant', color="b")
...
class Projections():
	...
	def get_random_proj(self):
		...
		for i in range(0, 2):
			...
			for j in range(0, 729):
				random_element = np.random.normal()
				row.append(random_element)
			matrix.append(row)
		return matrix
	...
	def pca_projection(self,X,Y):
		...
		cov_X = compute_covariance_matrix(X, X)
		U, D, V = svd(cov_X)
		want = (U.T)[0:2]
		return want
